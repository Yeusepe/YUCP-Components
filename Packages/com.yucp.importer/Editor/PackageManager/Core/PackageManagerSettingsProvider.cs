...
namespace YUCP.Importer.Editor.PackageManager
{
    internal sealed class PackageManagerSettingsProvider : SettingsProvider
    {
        ...
        public override void OnActivate(string searchContext, VisualElement rootElement)
        {
            ...
            var serverDescription = new Label("Choose which trusted URL the importer should use for Creator Identity sign-in and license verification.");
            ...
            void RefreshServerStatus()
            {
                ...
                {
                    serverStatus.text = "This URL is not in the trusted URL list yet. Add it below before using it for sign-in or verification.";
                    serverStatus.messageType = HelpBoxMessageType.Warning;
                }
                else
                {
                    serverStatus.text = "This trusted URL will be used by the importer for Creator Identity and license verification.";
                    serverStatus.messageType = HelpBoxMessageType.Info;
                }
            }
            ...
            var trustTitle = new Label("Trusted URLs");
            trustTitle.style.unityFontStyleAndWeight = FontStyle.Bold;
            trustTitle.style.marginBottom = 4;
            rootElement.Add(trustTitle);

            var trustDescription = new Label("Only trusted URLs should be used to fetch authority data or to run Creator Identity sign-in for this project.");
            ...
            var fetchHelp = new HelpBox("When you add or refresh a trusted URL, fetched authority keys are cached and package verification trust is reloaded immediately.", HelpBoxMessageType.Info);
            ...
            void RefreshTrustedUrlsList()
            {
                ...
                if (urls.Count == 0)
                {
                    var empty = new HelpBox("No trusted URLs configured yet. Add your local or production authority URLs here.", HelpBoxMessageType.Warning);
        ...
        private static void RefreshTrustedUrl(string url)
        {
            ...
            {
                EditorUtility.DisplayDialog("Failed to Refresh Trusted URL", $"Could not fetch authority keys from '{url}'.\n\n{result.error}", "OK");
            }
        }
