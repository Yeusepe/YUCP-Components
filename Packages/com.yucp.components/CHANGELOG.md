...
### Changed
- Advanced Viseme's no-tracker decoder now renders a corpus-fitted, duration-conditioned probability-simplex trajectory directly instead of sending a moving target through the older two-pole speech observer. Five positive controls form a 168 ms cubic plus a 56 ms linear tail, while 72 ms destination-interruptible state transitions preserve continuity through rapid `A -> B -> A` and `A -> B -> C` changes. Nested speaker cross-validation selected this timing in every fold and cut false plateaus by 64-67% relative to the earlier 128 ms direct cubic, with lower first- through third-difference error and a documented small pose-error tradeoff. Linear articulation rows are commuted into the decoder so the calibrated basis and residual consume the same temporal epoch. The hard index still cannot recover Oculus's hidden 15 weights; active face-tracking fusion, the exact full-authority endpoint, synced parameters, and source meshes are unchanged.
