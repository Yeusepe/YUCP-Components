...
`Avatar Menu` can generate saved radial sliders under `YUCP/Viseme Settings`. The generated root contains a one-page `Simple` menu with friendly names—including `Speech Liveliness`—and an `Advanced` branch containing the existing Speech, Tracking, Phonetics, and Tongue submenus. Both branches point to the same full-precision local Float parameters. With `Share Settings` enabled, one 8-bit data carrier and five stable channel-index bits transmit every applicable slider for a fixed **13 synced bits** instead of 208 bits for 26 independent Floats. Remote copies use 255 evenly spaced values over 0..1, while the wearer remains immediate and unquantized. The open radial is prioritized and an automatic background carousel repairs packet loss and fills late joiners; the transport adds one lightweight state-machine layer and no BlendTrees. The source Floats remain saved, while hidden carriers are deliberately unsaved. `Local Only` keeps the previous zero-bit behavior. `Silence Stability` controls the causal speech-memory release: the center uses the profile value, zero is an exact bypass, and the upper half retains an established talkspurt longer. Profile defaults remain the deterministic behavior seen where a local menu preference is unavailable.
