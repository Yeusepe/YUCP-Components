...
{
    [CustomEditor(typeof(RotationDampingConstraintData))]
    public class RotationDampingConstraintDataEditor : UnityEditor.Editor
    {
        private RotationDampingConstraintData data;
        ...
        private static readonly string WikiUrl = "https://github.com/Yeusepe/Yeusepes-Modules/wiki/Rotation-Damping-Constraint";
        ...
        public override VisualElement CreateInspectorGUI()
        {
            ...
            root.Add(YUCP.Components.Resources.YUCPComponentHeader.CreateHeaderOverlay("Rotation Damping Constraint"));
            
            var betaWarning = BetaWarningHelper.CreateBetaWarningVisualElement(typeof(RotationDampingConstraintData));
            if (betaWarning != null) root.Add(betaWarning);
            
            var supportBanner = SupportBannerHelper.CreateSupportBannerVisualElement(typeof(RotationDampingConstraintData));
        ...
        private void UpdateOverviewCard(VisualElement container, RotationDampingConstraintData data, VRCAvatarDescriptor descriptor)
        {
            container.Clear();
            ...
            var groupingLabel = enableGroupingProp.boolValue
                ? RotationDampingConstraintData.NormalizeGroupId(constraintGroupIdProp.stringValue)
                : "Isolated (per-object)";
            
            var overviewCard = YUCPUIToolkitHelper.CreateCard("Rotation Damping Constraint Overview", null);
        ...
        private void UpdateDescriptorWarnings(VisualElement container)
        {
            ...
            {
                container.Add(YUCPUIToolkitHelper.CreateHelpBox("Attach Rotation Damping Constraint to the object you want to dampen, not the descriptor root.", YUCPUIToolkitHelper.MessageType.Warning));
            }
            else if (!data.transform.IsChildOf(descriptor.transform))
            {
                container.Add(YUCPUIToolkitHelper.CreateHelpBox("Rotation Damping Constraint target must be within the avatar hierarchy. Please move it inside the descriptor object.", YUCPUIToolkitHelper.MessageType.Error));
            }
        }
