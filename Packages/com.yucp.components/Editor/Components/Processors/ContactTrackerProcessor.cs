    ...
    public class ContactTrackerProcessor : IVRCSDKPreprocessAvatarCallback
    {
        ...
        private static bool BuildGroup(VRCAvatarDescriptor descriptor, GameObject prefab, AnimatorController sourceController, GroupKey key, List<GroupMember> members)
        {
            ...
            try
            {
                ...
                System.IO.Directory.CreateDirectory("Assets/VRLabs/GeneratedAssets/ContactTracker/Animators/");
                string uniqueControllerPath = AssetDatabase.GenerateUniqueAssetPath("Assets/VRLabs/GeneratedAssets/ContactTracker/Animators/ContactTracker.controller");
                ...
                if (mergedController == null)
                {
                    Debug.LogError("[YUCP Contact Tracker] Failed to create controller.");
                    foreach (var member in members)
                    {
                        member.Component.SetBuildSummary("Build failed");
                    }
